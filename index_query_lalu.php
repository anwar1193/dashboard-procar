<?php  
	...
	//Ambil bulan sales
	$bulan_sales = $bulan_now;
	$tahun_sales = $tahun_now;
	
	//Ambil tanggal Ayda
	$q_tglAyda = "SELECT * FROM tbl_tgl_ayda order by id DESC LIMIT 0,1";
	$r_tglAyda = mysqli_query($koneksi, $q_tglAyda) or die ('error fungsi3');
	...
	$query_sales = "SELECT SUM(IF( Bulan = '$bulan_sales' AND Tahun = $tahun_sales, TotalPenjualan, 0)) AS penjualan
						FROM tbl_sales";
	$result_sales = mysqli_query($koneksi,$query_sales) or die ('error fungsi4');
	...
	$result_tar_sales = mysqli_query($koneksi,$query_tar_sales) or die ('error fungsi5');
	...
	$query_collpay = "SELECT * FROM tbl_coll_paymentlalu WHERE cabang='TOTAL'";
	$res_collpay = mysqli_query($koneksi,$query_collpay) or die ('error fungsi6');
	...
	$query_tunggakan = "SELECT SUM(IF( DPD<=0, OSPokokReal, 0)) AS current,
							...
							FROM tbl_collection2lalu";
	$result_tunggakan = mysqli_query($koneksi,$query_tunggakan) or die ('error fungsi7');
