<?php
	...
	if($bulan_now=='01'){
		$query_tar_sales = "SELECT JAN AS target FROM tbl_target WHERE BranchName=$cabang";
	}else if($bulan_now=='02'){
		$query_tar_sales = "SELECT FEB AS target FROM tbl_target WHERE BranchName=$cabang";
	}else if($bulan_now=='03'){
		$query_tar_sales = "SELECT MAR AS target FROM tbl_target WHERE BranchName=$cabang";
	}else if($bulan_now=='04'){
		$query_tar_sales = "SELECT APR AS target FROM tbl_target WHERE BranchName=$cabang";
	}else if($bulan_now=='05'){
		$query_tar_sales = "SELECT MEI AS target FROM tbl_target WHERE BranchName=$cabang";
	}else if($bulan_now=='06'){
		$query_tar_sales = "SELECT JUN AS target FROM tbl_target WHERE BranchName=$cabang";
	}else if($bulan_now=='07'){
		$query_tar_sales = "SELECT JUL AS target FROM tbl_target WHERE BranchName=$cabang";
	}else if($bulan_now=='08'){
		$query_tar_sales = "SELECT AGU AS target FROM tbl_target WHERE BranchName=$cabang";
	}else if($bulan_now=='09'){
		$query_tar_sales = "SELECT SEP AS target FROM tbl_target WHERE BranchName=$cabang";
	}else if($bulan_now=='10'){
		$query_tar_sales = "SELECT OKT AS target FROM tbl_target WHERE BranchName=$cabang";
	}else if($bulan_now=='11'){
		$query_tar_sales = "SELECT NOV AS target FROM tbl_target WHERE BranchName=$cabang";
	}else if($bulan_now=='12'){
		$query_tar_sales = "SELECT DES AS target FROM tbl_target WHERE BranchName=$cabang";
	}
	...
	$query_tunggakan = "SELECT SUM(IF( DPD<=0, OSPokokReal, 0)) AS current,
							...
							SUM(IF( DPD BETWEEN 31 AND 90, OSPokokReal, 0)) AS tunggakan3,
							SUM(IF( DPD>=91, OSPokokReal, 0)) AS tunggakan4,
							SUM(OSPokokReal) AS total_tunggakan
							FROM tbl_collection WHERE BranchName=$cabang";
	...
	$t31_90 = $row_tunggakan['tunggakan3'];
	$pers31_90 = ($t31_90/$total_tung) * 100;
	...
	// 1. Ayda Out
		...
		//OS
		$qayda_out_os = "SELECT SUM(os_ayda) AS total_penjualan FROM tbl_ayda_out WHERE cabang=$cabang";
		...
		//Unit
		$qayda_out_unit = "SELECT COUNT(*) AS total_penjualan_unit FROM tbl_ayda_out WHERE cabang=$cabang";
	...
	// 2. Ayda In
		...
		$qayda_in_os =  "SELECT 
							...
							FROM tbl_ayda WHERE BranchName=$cabang";
		...
		//Unit
		$qayda_in_unit = "SELECT COUNT(*) AS penambahan_unit FROM tbl_ayda WHERE MONTH(CLOSETXNDATE)=$bulan_now AND YEAR(CLOSETXNDATE)=$tahun_now AND BranchName=$cabang";
	   ...
	   <div class="mother-grid-inner">
			...
			<!--header start here-->
				<div class="header-main" style="padding:2px">
					&nbsp;&nbsp;
					<h5 style="display: inline-block;">Dashboard PT Procar Finance (<?php echo $cabang; ?>)</h5>
...
<div class="row"> <!-- row content -->
	...
	<div class="col-md-3"> <!-- Colum grafik collectibility -->
		...
		<div class="tampil_coll" style="border:double;">
			<div style="width: 250px; height: 190px; margin: 0 auto;"> <!-- Lebar Area Seluruh Grafik -->
				<div id="collection" style="width: 240px; height: 190px; float: left"></div>
			</div>
		</div>
	</div>
	...
	<div class="col-md-3"> <!-- Colum grafik collection (tunggakan) -->
		...
		<div class="tampil_collectibility" style="border:double;">
			<div class="market-update-block clr-block-3" style="padding:5px">
				<div class="col-md-12 market-update-left">
					...
					<p id="header_4">TUNGGAKAN <?php echo $nama_bulan; ?> <?php echo $tahun_now; //sebelumnya nama_bulan?></p>
		...
		<div class="" style="border:double;"><br />
			<div style="width: 250px; height: 170px; margin: 0 auto; "> <!-- Lebar Area Seluruh Grafik -->
				...
				<p id="teks"><b>&nbsp;31 - 90</b></p>
				<div id="box_31_90a"><?php echo substr($pers31_90,0,4);?>%</div> 
				<div id="box_31_90b"><?php echo rupiah($t31_90); ?></div>
				...
				<div id="total_b"><?php echo rupiah($current+$t1_7+$t8_30+$t31_90+$t91_up); ?></div>
		...
		<div class="tampil_sales" style="border:double;">
			<div class="market-update-block clr-block-4" style="padding:5px">
				<div class="col-md-12 market-update-left">
						...
						<p id="header_4">AYDA <?php echo $nama_bulan; ?> <?php echo $tahun_now; //sebelumnya nama_bulan ?></p>
...
<div class="row">
	...
	<div class="col-md-6">
		...
		Highcharts.chart('portfolio-sales', {
			...
			title: {
				text: '<style="font-size:15px">Portfolio & Sales 2020</style>'
			},
			...
			series: [{
				...
				data: [
					...
					<?php
						...
						$q_port = "SELECT * FROM tbl_portfolio WHERE cabang=$cabang";
	...
	<div class="col-md-6">
		...
		Highcharts.chart('collectibility', {
			...
			title: {
				text: '<style="font-size:15px">Collectibility <?php echo $tahun_now ?></style>'
			},
			credits: { //untuk menghilangkan link highchart.com di bawah grafik
				enabled: false
			},
			...
			series: [{
				...
				data: [
					...
					<?php
						...
						$q_coll_nas = "SELECT * FROM tbl_coll_nas WHERE cabang=$cabang";
				...
				data: [
					<?php
						...
						$q_coll_nas2 = "SELECT * FROM tbl_coll_nas WHERE cabang=$cabang";
		...
		<script type="text/javascript">
			...
			Highcharts.chart('ayda_perbulan', {
				...
				title: {
					text: '<style="font-size:15px">Ayda 2019</style>'
				},
