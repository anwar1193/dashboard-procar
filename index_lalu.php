<?php
	...
	$bulan = date('m', time()-60*60*24*30);
	$tanggal_now = date('d', time()-60*60*24*30);
	$bulan_now = date('m', time()-60*60*24*30);
	$tahun_now = date("Y", time()-60*60*24*30);
	
	include 'index_query_lalu.php';
	   ...
	   <div class="mother-grid-inner">
			...
			<!--header start here-->
				<div class="header-main" style="padding:2px">
					&nbsp;&nbsp;
					<h5 style="display: inline-block;">Dashboard PT Procar Finance (Data Bulan Lalu)</h5>
					<!-- Button Peta modal -->
					
					<!-- Button Data Bulan Berjalan -->
					<a href="index.php" class="btn btn-success btn-xs" style="display: inline-block;">Bulan Berjalan</a>
		...
		<div class="tampil_collectibility" style="border:double;">
			<div class="market-update-block" style="padding:5px; background-color: #3333cc">
				<div class="col-md-12 market-update-left">
					...
					<p id="header_4">TUNGGAKAN <?php echo $nama_bulan.' '.$tahun_sales; //sebelumnya $nama_bulan;?></p>
			...
			series: [{
				...
				data: [
					...
					<?php
						...
						// $q_port = "SELECT * FROM tbl_portfolio WHERE cabang='TOTAL' AND tahun='$tahun_now'";

						$q_port = "SELECT SUM(jan) AS jan, SUM(feb) AS feb, SUM(mar) AS mar, SUM(apr) AS apr, SUM(mei) AS mei,
									SUM(jun) AS jun, SUM(jul) AS jul, SUM(agu) AS agu, SUM(sep) AS sep, SUM(okt) AS okt,
									SUM(nov) AS nov, SUM(des) AS des FROM tbl_portfolio WHERE tahun='$tahun_now'";
				...
				data: [
					...
					$bulan_sales_sekarang = date('m');
				
					// sales tahun ini
					$query_sales = "SELECT 
										SUM(IF( Bulan = 01 AND Tahun = '$tahun_now' AND Bulan != $bulan_sales_sekarang, TotalPenjualan, 0)) AS penjualan_1,
										SUM(IF( Bulan = 02 AND Tahun = '$tahun_now' AND Bulan != $bulan_sales_sekarang, TotalPenjualan, 0)) AS penjualan_2,
										SUM(IF( Bulan = 03 AND Tahun = '$tahun_now' AND Bulan != $bulan_sales_sekarang, TotalPenjualan, 0)) AS penjualan_3,
										SUM(IF( Bulan = 04 AND Tahun = '$tahun_now' AND Bulan != $bulan_sales_sekarang, TotalPenjualan, 0)) AS penjualan_4,
										SUM(IF( Bulan = 05 AND Tahun = '$tahun_now' AND Bulan != $bulan_sales_sekarang, TotalPenjualan, 0)) AS penjualan_5,
										SUM(IF( Bulan = 06 AND Tahun = '$tahun_now' AND Bulan != $bulan_sales_sekarang, TotalPenjualan, 0)) AS penjualan_6,
										SUM(IF( Bulan = 07 AND Tahun = '$tahun_now' AND Bulan != $bulan_sales_sekarang, TotalPenjualan, 0)) AS penjualan_7,
										SUM(IF( Bulan = 08 AND Tahun = '$tahun_now' AND Bulan != $bulan_sales_sekarang, TotalPenjualan, 0)) AS penjualan_8,
										SUM(IF( Bulan = 09 AND Tahun = '$tahun_now' AND Bulan != $bulan_sales_sekarang, TotalPenjualan, 0)) AS penjualan_9,
										SUM(IF( Bulan = 10 AND Tahun = '$tahun_now' AND Bulan != $bulan_sales_sekarang, TotalPenjualan, 0)) AS penjualan_10,
										SUM(IF( Bulan = 11 AND Tahun = '$tahun_now' AND Bulan != $bulan_sales_sekarang, TotalPenjualan, 0)) AS penjualan_11,
										SUM(IF( Bulan = 12 AND Tahun = '$tahun_now' AND Bulan != $bulan_sales_sekarang, TotalPenjualan, 0)) AS penjualan_12
										FROM tbl_sales";
					$result_sales = mysqli_query($koneksi,$query_sales) or die ('error fungsi');
					$row_sales = mysqli_fetch_assoc($result_sales);
					
					// Tampilkan Sales
					echo $row_sales['penjualan_1'].',';
					echo $row_sales['penjualan_2'].',';
					echo $row_sales['penjualan_3'].',';
					echo $row_sales['penjualan_4'].',';
					echo $row_sales['penjualan_5'].',';
					echo $row_sales['penjualan_6'].',';
					echo $row_sales['penjualan_7'].',';
					echo $row_sales['penjualan_8'].',';
					echo $row_sales['penjualan_9'].',';
					echo $row_sales['penjualan_10'].',';
					echo $row_sales['penjualan_11'].',';
					echo $row_sales['penjualan_12'];
			...
			series: [{
				...
				data: [
					...
					<?php
						...
						$q_coll_nas = "SELECT * FROM tbl_coll_nas WHERE cabang='TOTAL' AND tahun='$tahun_now'";
						$r_coll_nass = mysqli_query($koneksi,$q_coll_nas) or die ('error coll nas');
						$row_coll_nas = mysqli_fetch_assoc($r_coll_nass);
				...
				data: [
					<?php
						...
						$q_coll_nass = "SELECT * FROM tbl_coll_nas WHERE cabang='TOTAL' AND tahun='$tahun_now'";
			...
			Highcharts.chart('ayda_perbulan', {
				...
				series: [{
					...
					data: [
						...
						<?php
								$bulan_ayda_sekarang = date('m');
				
								// data AYDA-IN tahun ini
								$q_ayda_in = "SELECT 
													SUM(IF( bulan = 1 AND tahun = '$tahun_now' AND bulan != $bulan_ayda_sekarang, penambahan_os, 0)) AS ayda1,
													SUM(IF( bulan = 2 AND tahun = '$tahun_now' AND bulan != $bulan_ayda_sekarang, penambahan_os, 0)) AS ayda2,
													SUM(IF( bulan = 3 AND tahun = '$tahun_now' AND bulan != $bulan_ayda_sekarang, penambahan_os, 0)) AS ayda3,
													SUM(IF( bulan = 4 AND tahun = '$tahun_now' AND bulan != $bulan_ayda_sekarang, penambahan_os, 0)) AS ayda4,
													SUM(IF( bulan = 5 AND tahun = '$tahun_now' AND bulan != $bulan_ayda_sekarang, penambahan_os, 0)) AS ayda5,
													SUM(IF( bulan = 6 AND tahun = '$tahun_now' AND bulan != $bulan_ayda_sekarang, penambahan_os, 0)) AS ayda6,
													SUM(IF( bulan = 7 AND tahun = '$tahun_now' AND bulan != $bulan_ayda_sekarang, penambahan_os, 0)) AS ayda7,
													SUM(IF( bulan = 8 AND tahun = '$tahun_now' AND bulan != $bulan_ayda_sekarang, penambahan_os, 0)) AS ayda8,
													SUM(IF( bulan = 9 AND tahun = '$tahun_now' AND bulan != $bulan_ayda_sekarang, penambahan_os, 0)) AS ayda9,
													SUM(IF( bulan = 10 AND tahun = '$tahun_now' AND bulan != $bulan_ayda_sekarang, penambahan_os, 0)) AS ayda10,
													SUM(IF( bulan = 11 AND tahun = '$tahun_now' AND bulan != $bulan_ayda_sekarang, penambahan_os, 0)) AS ayda11,
													SUM(IF( bulan = 12 AND tahun = '$tahun_now' AND bulan != $bulan_ayda_sekarang, penambahan_os, 0)) AS ayda12
													FROM tbl_ayda_asli";
					...
					data: [
						...
						<?php
								...
								$q_ayda_out = "SELECT 
													SUM(IF( bulan = 1 AND tahun = '$tahun_now' AND bulan != $bulan_ayda_sekarang, pengurangan_os, 0)) AS jualan_ayda1,
													SUM(IF( bulan = 2 AND tahun = '$tahun_now' AND bulan != $bulan_ayda_sekarang, pengurangan_os, 0)) AS jualan_ayda2,
													SUM(IF( bulan = 3 AND tahun = '$tahun_now' AND bulan != $bulan_ayda_sekarang, pengurangan_os, 0)) AS jualan_ayda3,
													SUM(IF( bulan = 4 AND tahun = '$tahun_now' AND bulan != $bulan_ayda_sekarang, pengurangan_os, 0)) AS jualan_ayda4,
													SUM(IF( bulan = 5 AND tahun = '$tahun_now' AND bulan != $bulan_ayda_sekarang, pengurangan_os, 0)) AS jualan_ayda5,
													SUM(IF( bulan = 6 AND tahun = '$tahun_now' AND bulan != $bulan_ayda_sekarang, pengurangan_os, 0)) AS jualan_ayda6,
													SUM(IF( bulan = 7 AND tahun = '$tahun_now' AND bulan != $bulan_ayda_sekarang, pengurangan_os, 0)) AS jualan_ayda7,
													SUM(IF( bulan = 8 AND tahun = '$tahun_now' AND bulan != $bulan_ayda_sekarang, pengurangan_os, 0)) AS jualan_ayda8,
													SUM(IF( bulan = 9 AND tahun = '$tahun_now' AND bulan != $bulan_ayda_sekarang, pengurangan_os, 0)) AS jualan_ayda9,
													SUM(IF( bulan = 10 AND tahun = '$tahun_now' AND bulan != $bulan_ayda_sekarang, pengurangan_os, 0)) AS jualan_ayda10,
													SUM(IF( bulan = 11 AND tahun = '$tahun_now' AND bulan != $bulan_ayda_sekarang, pengurangan_os, 0)) AS jualan_ayda11,
													SUM(IF( bulan = 12 AND tahun = '$tahun_now' AND bulan != $bulan_ayda_sekarang, pengurangan_os, 0)) AS jualan_ayda12
													FROM tbl_ayda_asli";
